...
def main_gameplay():
    ...
    while True:
        ...
        w = 0
        for digit in d:
            # w += game_image['number'][digit].get_width()
            w += game_image['numbers'][digit].get_width()
        Xoffset = (SCREEN_WIDTH - w) / 2

        for digit in d:
            display.blit(game_image['numbers'][digit], (Xoffset, SCREEN_HEIGHT * 0.12))
...
def get_random_pipes():
    ...
    y1 = pip_h - yes2 + off_s
    pipes = [
        {'x': pipeX, 'y': -y1}, # верхняя труба
        {'x': pipeX, 'y': yes2} # нижняя труба
    ]
    return pipes
